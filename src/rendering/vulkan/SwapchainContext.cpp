...
bool SwapchainContext::init(const DeviceContext& deviceContext, VkSurfaceKHR surface, uint32_t width, uint32_t height)
{
    VkPhysicalDevice physicalDevice = deviceContext.physicalDevice();
    VkDevice device = deviceContext.device();
    ...
    // Use provided dimensions, clamped to surface capabilities
    ...
    // Render pass (identical to GLFW overload)
    VkAttachmentDescription colorAttachment{};
    colorAttachment.format = imageFormat_;
    colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
    colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
